...
def _map_emails(fields):
    row = {}
    row["product_id"] = fields["alt_ref_id"][0]
    row["num"] = fields["id"][0]
...
def _query_emails(index, size, emails_query, additional_fields=[]):
    emails_resp = es().search(index=index, doc_type="emails", size=size,
                              fields=get_graph_row_fields() + additional_fields, body=emails_query)
    tangelo.log("es_query_utils._query_emails(total document hits = %s)" % emails_resp["hits"]["total"])

    return {"total": emails_resp["hits"]["total"],
            "hits": [_map_emails(hit["fields"]) for hit in emails_resp["hits"]["hits"]]}
