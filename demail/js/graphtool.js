...
).on("mouseout", function(d){tos = d.to.replace(/\./g,'_').replace(/@/g,'_').split(',');
for (i = 0; i < tos.length; i++)
{
d3.select("#" + d.from.replace(/\./g,'_').replace(/@/g,'_') + '_' + tos[i]).style("stroke", "#bbb");
		...
		var td = tr.selectAll("td")
  		.data(function(d){return [d.num + '::' + d.from + '::' + d.directory, d.datetime, d.from +'::' + d.fromcolor,d.to,d.cc,d.bcc,d.subject];})
  		.enter().append("td")
...
{
return d.replace(/,/g,', ');
}}).style("color", function(d,i) { if( i == 2) { 
return color(d.split('::')[1]);} else { return 'black';} }).style("stroke","#FFFFFF");
...
function drawGraph(graph){
           ...
           vis = svg.append('svg:g');
            ...
            .data(["end"]).enter().append("svg:marker").attr("id", String).attr("viewBox","0 -5 10 10").attr("refX",15).attr("refY",0).attr("markerWidth",7).attr("markerHeight",7).attr("orient", "auto").append("svg:path").attr("d", "M0,-5L10,0L0,5");
  			...
  			var link = vis.selectAll(".link")
    			...
    			.enter().append("path")
      			.attr("class", "link").attr("marker-end", "url(#end)")
                        .style("stroke", "black")
                        .style("stroke-width", function(d) { return .25 + (d[3] / 400); })
                        .attr("id",function(d) {return d[0].name.replace(/\./g,'_').replace(/@/g,'_') + '_' + d[2].name.replace(/\./g,'_').replace(/@/g,'_');});
...
function get_component_by_number(comp){
        ...
        $.getJSON("get_comp_service?text=" + encodeURIComponent(text), function (graph) {
           ...
           vis = svg.append('svg:g');
			...
			vis.selectAll("svg:circle").on("mouseover", function() { d3.select(this).select("svg text").style("opacity","100") })
